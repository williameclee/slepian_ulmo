...
assert(isequal(domain.Inclination, [-90, 90]));
...
assert((domain.Upscale) == 0);
assert((domain.Buffer) == 0);
assert(isequal(domain.Inclination, [-60, 60]));
...
domain = GeoDomain('invaliddomain', 'Upscale', -1, 'Buffer', 1, ...
    'Inclination', [-30, 30], 'MoreBuffers', {'earthquakes', 10}, ...
    'NearBy', true);
assert(strcmp(domain.Domain, 'invaliddomain'));
assert((domain.Upscale) == 0);
assert((domain.Buffer) == 1);
assert(isequal(domain.Inclination, [-30, 30]));
assert(isequal(domain.MoreBuffers, {'earthquakes', 10}));
assert(~domain.NearBy);
...
assert(isequal(domain(1).Inclination, [-90, 90]));
assert(isequal(domain(2).Inclination, [-60, 60]));
...
disp('All GeoDomain test passed')
