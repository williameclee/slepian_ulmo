...
function varargout = grace2slept_new(varargin)
    ...
    [sphCoeffs, cal_errors, thedates] = ...
        grace2plmt(dataCentre, releaseLevel, Ldata, unit, ...
        'Deg1Correction', deg1correction, 'C20Correction', c20correction, 'C30Correction', c30correction); %#ok<ASGLU>
...
function varargout = parseinputs(vArargin)
    ...
    addOptional(p, 'DataProduct', dataProductDefault, ...
        ...
        && ischar(x{1}) && ischar(x{2}) && isnumeric(x{3}) || isempty(x));
...
function [outputFilePath, region] = getoutputfile(region, buf, moreRegionSpecs, Lwindow, ...
            ...
            switch regiontype(region)
                case 'geographical'
                    ...
                    if iscell(region) && length(region) >= 3
                        ...
                        regionString = char([region, dataattrchar('Buffer', buf, moreRegionSpecs{:})]);
    ...
    if ~deg1correction
        outputFileName = strrep(outputFileName, '.mat', '-nDeg1.mat');
    end
    if ~c20correction
        outputFileName = strrep(outputFileName, '.mat', '-nC20.mat');
    end

    if ~c30correction
        outputFileName = strrep(outputFileName, '.mat', '-nC30.mat');
    end
