...
function varargout = parseinputs(varargin)
    ...
    p = inputParser;
    addOptional(p, 'Product', dfOpt.Product, ...
        @(x) (iscell(x) && length(x) == 3) || isempty(x));
    addOptional(p, 'Domain', dfOpt.Domain, ...
        @(x) (ischar(x)) || isstring(x) || iscell(x) || ...
        isa(x, 'GeoDomain') || (isnumeric(x) && size(x, 2) == 2) || ...
        (isempty(x)));
    addOptional(p, 'L', LD, ...
        @(x) (isnumeric(x) && (length(x) <= 2)) || (isempty(x)));
    addOptional(p, 'phi', dfOpt.CapSpecs, ...
        @(x) (isnumeric(x) && isscalar(x)) || (isempty(x)));
    addOptional(p, 'theta', dfOpt.CapSpecs, ...
        @(x) (isnumeric(x) && isscalar(x)) || (isempty(x)));
    addOptional(p, 'omega', dfOpt.CapSpecs, ...
        @(x) (isnumeric(x) && isscalar(x)) || (isempty(x)));
    addOptional(p, 'Truncation', dfOpt.Truncation, ...
        @(x) ((isnumeric(x) && isscalar(x) && x > 0) || ...
        strcmp(x, 'N')) || (isempty(x)));
    addOptional(p, 'Unit', dfOpt.Unit, ...
        @(x) ischar(x) || isempty(x));
    addOptional(p, 'TimeRange', [], ...
        @(x) isempty(x) || ((isdatetime(x) || isnumeric(x))));
    addOptional(p, 'ForceNew', dfOpt.ForceNew, ...
        @(x) ((isnumeric(x) || islogical(x)) && isscalar(x)) || isempty(x));
    addOptional(p, 'MoreRegionSpecs', {}, @iscell);
    addParameter(p, 'SaveData', [], ...
        @(x) ((isnumeric(x) || islogical(x)) && isscalar(x)) || isempty(x));
    addParameter(p, 'BeQuiet', false, ...
        @(x) (isnumeric(x) || islogical(x)) && isscalar(x));
    addParameter(p, 'Deg1Correction', [], ...
        @(x) ((isnumeric(x) || islogical(x)) && isscalar(x)) || isempty(x));
    addParameter(p, 'C20Correction', [], ...
        @(x) ((isnumeric(x) || islogical(x)) && isscalar(x)) || isempty(x));
    addParameter(p, 'C30Correction', [], ...
        @(x) ((isnumeric(x) || islogical(x)) && isscalar(x)) || isempty(x));
    addParameter(p, 'RecomputeDegree1', [], ...
        @(x) ((isnumeric(x) || islogical(x)) && isscalar(x)) || isempty(x));
    ...
    product = conddefval(p.Results.Product, dfOpt.Product);
    domain = conddefval(p.Results.Domain, dfOpt.Domain);
    L = conddefval(p.Results.L, dfOpt.Product);
    phi = conddefval(p.Results.phi, dfOpt.CapSpecs);
    theta = conddefval(p.Results.theta, dfOpt.CapSpecs);
    omega = conddefval(p.Results.omega, dfOpt.CapSpecs);
    unit = conddefval(p.Results.Unit, dfOpt.Unit);
    domainSpecs = p.Results.MoreRegionSpecs;
    J = conddefval(p.Results.Truncation, dfOpt.Truncation);
    timeRange = p.Results.TimeRange;
    forceNew = conddefval(logical(p.Results.ForceNew), dfOpt.ForceNew);
    saveData = conddefval(logical(p.Results.SaveData), dfOpt.SaveData);
    beQuiet = logical(p.Results.BeQuiet);
    deg1corr = logical(p.Results.Deg1Correction);
    c20corr = logical(p.Results.C20Correction);
    c30corr = logical(p.Results.C30Correction);
    redoDeg1 = logical(p.Results.RecomputeDegree1);
    outputFmt = p.Results.OutputFormat;
    timeFmt = p.Results.TimeFormat;
    ...
    if (~isempty(redoDeg1) || ~redoDeg1) && ...
            (~isempty(p.Results.SaveData) && p.Results.SaveData)
        warning('%s:CannotSaveData', ...
